...
class DELIVER(Dataset):
    ...
    def __init__(self, root: str = '/dataset/DELIVER', split: str = 'train', transform = None, modals = ['img'], miss = True, case = None) -> None:
        super().__init__()
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, Tensor]:
        ...
        x1 = rgb.replace('/img', '/depth').replace('_rgb', '_depth')
        # x1 = rgb.replace('/img', '/missing').replace('_rgb', '_missing')
        ...
        if self.split == 'train' and self.miss:
            ...
            if rgb_miss == depth_miss:
                # print('No miss')
                x1 = rgb.replace('/img', '/depth').replace('_rgb', '_depth')
            elif (rgb_miss == 1) and (depth_miss == 0):
                # print('RGB miss')
                x1 = rgb.replace('/img', '/depth').replace('_rgb', '_depth')
                rgb = rgb.replace('/img', '/missing').replace('_rgb', '_missing')
...
if __name__ == '__main__':
    ...
    for case in cases:
        ...
        trainset = DELIVER(transform=traintransform, split='train', modals=['img', 'depth'], case=case)
