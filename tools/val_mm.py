...
def evaluate(model, dataloader, device):
    ...
    for images, labels in tqdm(dataloader):
        ...
        if sliding:
            ...
            # preds = model(images)['pred'].softmax(dim=1)
...
def main(cfg):
    ...
    for case in cases:
        dataset = eval(cfg['DATASET']['NAME'])(cfg['DATASET']['ROOT'], 'val', transform, cfg['DATASET']['MODALS'], cfg['DATASET']['MISS'],case)
        # --- test set
        # dataset = eval(cfg['DATASET']['NAME'])(cfg['DATASET']['ROOT'], 'test', transform, cfg['DATASET']['MODALS'], case)
        model = eval(cfg['MODEL']['NAME'])(cfg['MODEL']['BACKBONE'], dataset.n_classes, cfg['DATASET']['MODALS'], eval_cfg['IMAGE_SIZE'])
