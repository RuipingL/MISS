...
from semseg.optimizers_multimae import param_groups_lrd, adjust_learning_rate
from semseg.utils.utils import fix_seeds, setup_cudnn, cleanup_ddp, setup_ddp, get_logger, cal_flops, print_iou
from val_mm import evaluate
import shutil
# from semseg.models.cmnext import load_dualpath_model
...
def main(cfg, gpu, save_dir):
    ...
    trainset = eval(dataset_cfg['NAME'])(dataset_cfg['ROOT'], 'train', traintransform, dataset_cfg['MODALS'], dataset_cfg['MISS'])
    valset = eval(dataset_cfg['NAME'])(dataset_cfg['ROOT'], 'val', valtransform, dataset_cfg['MODALS'])
    class_names = trainset.CLASSES

    model = eval(model_cfg['NAME'])(model_cfg['BACKBONE'], trainset.n_classes, dataset_cfg['MODALS'], train_cfg['IMAGE_SIZE'])
    ...
    param_groups = param_groups_lrd(model, optim_cfg['WEIGHT_DECAY'],
                                        no_weight_decay_list=model.no_weight_decay(),
                                        layer_decay=0.75 # multimae
                                        )
    optimizer = torch.optim.AdamW(param_groups, lr=lr) # CoMAE: https://github.com/MCG-NJU/CoMAE/blob/main/main_finetune.py
    ...
    if (train_cfg['DDP'] and torch.distributed.get_rank() == 0) or (not train_cfg['DDP']):
        ...
        # cal_flops(model, dataset_cfg['MODALS'], logger) # TODO: fix this
    ...
    for epoch in range(start_epoch, epochs):
        ...
        # pbar = tqdm(enumerate(trainloader), total=iters_per_epoch, desc=f"Epoch: [{epoch+1}/{epochs}] Iter: [{0}/{iters_per_epoch}] Loss: {train_loss:.8f}")
        for iter, (sample, lbl) in pbar:
            ...
            # pbar.set_description(f"Epoch: [{epoch+1}/{epochs}] Iter: [{iter+1}/{iters_per_epoch}] LR: {max_lr:.8f} Loss: {train_loss / (iter+1):.8f}")
...
if __name__ == '__main__':
    ...
    date = time.strftime("%Y-%m-%d_frozen")
    ...
    exp_name = '_'.join([cfg['DATASET']['NAME'], model, modals, date])
    save_dir = Path(cfg['SAVE_DIR'], exp_name)
    if os.path.isfile(cfg['MODEL']['RESUME']):
        save_dir =  Path(os.path.dirname(cfg['MODEL']['RESUME']))
    os.makedirs(save_dir, exist_ok=True)
    shutil.copy(args.cfg, os.path.join(save_dir, 'config.yaml'))
